...
mongoose.connect("mongodb://localhost:27017/cubeUsersDB", {
      ...
      User.findById(req.user.id, function (err, foundUser) {
        if (!err) {
          if (foundUser) {
            ...
            if (
              currentLeastTime === undefined ||
              currentLeastTime > solveTime
            ) {
              foundUser.leastTime = solveTime;
            }

            if (
              currentLeastSteps === undefined ||
              currentLeastSteps > solveSteps
            ) {
...
app.get("/results", function (req, res) {
  User.find(
    { leastTime: { $ne: undefined }, leastSteps: { $ne: undefined } },
    function (err, foundUsers) {
      var leastStepsArray = foundUsers.map(function (a) {
        return [a.username, a.leastSteps];
      });
      ...
      leastStepsArray.sort(function (a, b) {
        return a[1] - b[1];
      });
      leastTimeArray.sort(function (a, b) {
        return a[1] - b[1];
      });
      ...
      res.render("results", {
        bestTimes: leastTimeArray,
        leastSteps: leastStepsArray,
      });
    }
  );
});
